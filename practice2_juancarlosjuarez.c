...
// Device Driver for the Parallel Port
...
NTSTATUS DispatchRead(PDEVICE_OBJECT fdo, PIRP Irp)
	...
	unsigned char mask_status[] = {S_BUSY, S_PAPER_END, S_SELECT_IN, S_nERROR, S_BUSY, S_PAPER_END, S_SELECT_IN, S_nERROR};

	unsigned char status;
	unsigned char MSK_CTRL = 0x0B;
	unsigned char MSK_STS = 0x80;
	...
	for(i = 0; i < 8; i++, aux<<=1){
		if(i == 0){
			...
			status = READ_PORT_UCHAR((unsigned char *)0x379)^MSK_STS;
		} else if(i == 4){
			control = READ_PORT_UCHAR((unsigned char *)0x37A)^MSK_CTRL;
	...
	*pDato = pdx -> globalDato; // We simulate this part as all of this is just simulated (should be equal to dato)
...
NTSTATUS DispatchWrite(PDEVICE_OBJECT fdo, PIRP Irp)
	...
	unsigned char digits[] = {0x3F,0x06,0xDA,0x4F,0x66,0x6C,0x7D,0x07,0x7F, 0x6F};

	control = READ_PORT_UCHAR((unsigned char *)0x37A)^MSK_CTRL;
	control = control & 0xFE;
	WRITE_PORT_UCHAR((unsigned char *)0x37A,control^MSK_CTRL);

	unsigned char finalInformation;

	if(dato > 9){
		WRITE_PORT_UCHAR((unsigned char *)0x378, 0xFF);
		finalInformation = 0xFF;
	}else{
		WRITE_PORT_UCHAR((unsigned char *)0x378, digits[dato]);
		finalInformation = digits[dato];
	}
	...
	Irp->IoStatus.Status = STATUS_SUCCESS;
	Irp->IoStatus.Information = finalInformation;
	return STATUS_SUCCESS;
}							// DispatchWrite
